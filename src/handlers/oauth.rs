...
pub async fn handle_oauth_callback(
    ...
    let (provider_id, username_hint) = match provider.as_str() {
        "github" => {
            let res = client
                ...
                .await
                .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            let user: GitHubUser = res.json().await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            (user.id.to_string(), user.login.unwrap_or_else(|| user.id.to_string()))
        }
        "google" => {
            let res = client
                ...
                .await
                .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            let user: GoogleUser = res.json().await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            (user.id.clone(), user.email.unwrap_or_else(|| user.id.clone()))
        }
    ...
    match user {
        Some(u) => {
            session.insert("user_id", u.id).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            Ok(Redirect::to("/"))
        }
        None => {
            ...
            sqlx::query("INSERT INTO oauth_accounts (user_id, provider, provider_id) VALUES (?, ?, ?)")
                .bind(new_user.id)
                .bind(&provider)
                .bind(&provider_id)
                .execute(&state.pool)
            ...
            session.insert("user_id", new_user.id).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            tracing::info!("Created new OAuth user: {} via {}", actual_username, provider);
            Ok(Redirect::to("/"))
        }
    }
}
